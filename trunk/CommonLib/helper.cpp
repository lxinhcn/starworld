#include "stdafx.h"
#include "helper.h"
bool GetModulePath( HMODULE hModule, TCHAR* pszPath, DWORD nOutSize )
{
	DWORD nLen = GetModuleFileName( hModule, pszPath, nOutSize );

	if( nLen )
	{
		while( nLen && pszPath[nLen--] != '\\' );
		pszPath[nLen+1] = 0;
		return true;
	}
	return false;
...
CErrorDescription::CErrorDescription()
...
_lpctstr CErrorDescription::GetDescription()
{
	return (_lpctstr)lpszDescription;
}

DWORD CErrorDescription::GetErrorCode()
{
	return dwErrorCode;
}
...
RTCCheck::RTCCheck()
{
	m_checkValue = 0xfbfbfbfb;
}

RTCCheck::~RTCCheck()
{
	ASSERT( m_checkValue == 0xfbfbfbfb );
}
//////////////////////////////////////////////////////////////////////////
