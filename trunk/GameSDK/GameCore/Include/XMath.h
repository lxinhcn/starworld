...
namespace XGC
{
	...
	namespace XMath
	{
		...
		CORE_API  extern const float			INFINITY;					// huge number which should be larger than any valid number used
		...
		CORE_API  extern const int			MAX_INTEGER;
		CORE_API  extern const int			MIN_INTEGER;
		...
		CORE_API __inline int	Sign (int iValue);
		...
		CORE_API __inline float	UnitRandom (unsigned int uiSeed = 0);
		...
		CORE_API __inline float	SymmetricRandom (unsigned int uiSeed = 0);
		...
		CORE_API __inline float	IntervalRandom (float fMin, float fMax,	unsigned int uiSeed = 0);
