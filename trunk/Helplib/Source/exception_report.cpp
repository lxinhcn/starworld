...
WheatyExceptionReport::WheatyExceptionReport( )   // Constructor
{
    ...
    if ( pszDot )
    {
        ...
        if ( _tcslen(pszDot) >= 3 )
            _tcscpy( pszDot, _T("txt") );   // "RPT" -> "Report"
    }
        ...
        bool bWriteVariables )  // true if local/params should be output
{
    ...
    while ( 1 )
    {
        ...
        {
			USES_CONVERSION;
			TCHAR* p = A2T(lineInfo.FileName);
            _tprintf( _T("  %s line %u"), p, lineInfo.LineNumber );
        }

        _tprintf( _T("\r\n") );

		PIMAGEHLP_SYMBOL pSym = (PIMAGEHLP_SYMBOL)symbolBuffer;
		ZeroMemory( pSym, sizeof( symbolBuffer ) );
		pSym->Size = sizeof( IMAGEHLP_SYMBOL );
		pSym->MaxNameLength = sizeof(symbolBuffer)/2 - sizeof( IMAGEHLP_SYMBOL );
		pSym->Address = sf.AddrPC.Offset;
		if( SymGetSymFromAddr( m_hProcess, sf.AddrPC.Offset, &dwLineDisplacement, pSym ) )
		{
			USES_CONVERSION;
			TCHAR* p = A2T( pSym->Name );
			_tprintf( _T("Function  %s line %u\r\n"), p, lineInfo.LineNumber );
		}
    ...
    PVOID         UserContext )
{
    ...
    __try
    {
		...
		{
			TCHAR* p = A2T( szBuffer );
            _tprintf( _T("\t%s\r\n"), p );
		}
    }
    __except( 1 )
    {
		TCHAR* p = A2T( pSymInfo->Name );
...
int __cdecl WheatyExceptionReport::_tprintf(const TCHAR * format, ...)
{
	...
	USES_CONVERSION;
	CHAR* p = T2A(szBuff);
    WriteFile(m_hReportFile, p, (DWORD)strlen(p), &cbWritten, 0 );
