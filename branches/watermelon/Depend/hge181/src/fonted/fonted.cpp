...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR lpszCommand, int)
{
	...
	if(hge->System_Initiate())
	{
		...
		if( lpszCommand )
		{
			...
			state.bItalic = false;
			state.nSize = 20;
			state.bAntialias = false;
			state.FontFamily = "ËÎÌו";
			CSymbolRange range[2];
			range[0].First	= 0;
			range[0].Last	= 255;
			range[1].First	= 0x8140;
			range[1].Last	= 0xFEA0;
			FontGenerate(state.FontFamily, state.nSize, state.nPadTop, state.nPadBtm, state.nPadLft, state.nPadRgt, state.bBold, state.bItalic, state.bAntialias, range, 2);
			cmdSaveFont();
		}
		else
		{
