...
#define GETLANG()		LOWORD(IME::m_hklCurrent)
...
#define INDICATOR_ENGLISH	0
#define INDICATOR_NON_IME	1
#define INDICATOR_CHS		2
#define INDICATOR_CHT		3
#define INDICATOR_KOREAN	4
#define INDICATOR_JAPANESE	5
...
public:
	...
	static short CheckInputLocale();
	static short IsImeInput();
	static void OnInputLangChange( HWND hWnd );
	...
	struct CCandList
	{
		...
		unsigned int	dwCount;       // Number of valid entries in the candidate list
		unsigned int	dwSelection;   // Currently selected candidate entry relative to page top
