...
LRESULT CALLBACK DXWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		bool btrapped = false;
		ImeUi_ProcessMessage( hWnd, uMsg, wParam, lParam, &btrapped );
		if( btrapped )
			return btrapped;
	}
...
void CALLBACK SendComposeChar( wchar_t c )
{
	MyGUI::InputManager::getInstance().injectKeyPress(MyGUI::KeyCode::Enum(c), MyGUI::Char(c));
}
...
namespace base
{
	...
	bool BaseManager::create()
	{
		...
		ImeUiCallback_OnChar = SendComposeChar;
