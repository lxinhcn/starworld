...
void hgeFont::Render(float x, float y, int align, const char *string)
{
	...
	if(align==HGETEXT_RIGHT) fx-=Ge_stringWidth(string, false);
	if(align==HGETEXT_CENTER) fx-=int(Ge_stringWidth(string, false)/2.0f);
	...
	while(*string)
	{
		if(*string=='\n')
		{
			...
			fx = x;
			if(align == HGETEXT_RIGHT)  fx -= Ge_stringWidth(string+1, false);
			if(align == HGETEXT_CENTER) fx -= int(Ge_stringWidth(string+1, false)/2.0f);
		}
		else
		{
...
float hgeFont::Ge_stringWidth(const char *string, bool bMultiline) const
{
	int i;
	float linew, w = 0;
