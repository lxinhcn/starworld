...
void TiXmlBase::Pu_string( const TIXML_STRING& str, TIXML_OSTREAM* stream )
{
	TIXML_STRING buffer;
	Pu_string( str, &buffer );
	(*stream) << buffer;
}

void TiXmlBase::Pu_string( const TIXML_STRING& str, TIXML_STRING* ou_string )
{
	int i=0;
	...
	while( i<(int)str.length() )
	{
			 ...
			 && str[i+2] == 'x' )
		{
			...
			{
				ou_string->append( str.c_str() + i, 1 );
				++i;
				if ( str[i] == ';' )
					break;
			}
		...
		{
			ou_string->append( entity[0].str, entity[0].strLength );
			++i;
		}
		else if ( c == '<' )
		{
			ou_string->append( entity[1].str, entity[1].strLength );
			++i;
		}
		else if ( c == '>' )
		{
			ou_string->append( entity[2].str, entity[2].strLength );
			++i;
		}
		else if ( c == '\"' )
		{
			ou_string->append( entity[3].str, entity[3].strLength );
			++i;
		}
		else if ( c == '\'' )
		{
			ou_string->append( entity[4].str, entity[4].strLength );
			++i;
		}
		else if ( c < 32 )
		{
			...
			ou_string->append( buf, (int)strlen( buf ) );
			++i;
		}
		else
		{
			//char realc = (char) c;
			//ou_string->append( &realc, 1 );
			*ou_string += (char) c;	// somewhat more efficient function call.
			++i;
		}
	}
}
...
int TiXmlElement::IntAttribute( const char* name, int nDefault ) const
{
...
{
	const char* pattr = Attribute(name);
	return pattr?atob(Attribute(name)):bDefault;
}
//////////////////////////////////////////////////////////////////////////
...
void TiXmlAttribute::StreamOut( TIXML_OSTREAM * stream ) const
{
	if (value.find( '\"' ) != TIXML_STRING::npos)
	{
		Pu_string( name, stream );
		(*stream) << "=" << "'";
		Pu_string( value, stream );
		(*stream) << "'";
	}
	else
	{
...
void TiXmlDeclaration::StreamOut( TIXML_OSTREAM * stream ) const
{
	...
	{
		(*stream) << "version=\"";
		Pu_string( version, stream );
		(*stream) << "\" ";
	}
	if ( !encoding.empty() )
	{
		(*stream) << "encoding=\"";
		Pu_string( encoding, stream );
		(*stream ) << "\" ";
	}
	if ( !standalone.empty() )
	{
		(*stream) << "standalone=\"";
		Pu_string( standalone, stream );
		(*stream) << "\" ";
	}
	(*stream) << "?>";
}
