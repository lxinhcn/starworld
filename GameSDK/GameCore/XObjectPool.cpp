...
namespace XGC
{
	...
	bool CXObjectPool::IsValidObject( xObject id )
	{
		object_handle &h = (object_handle&)id;
		if( h.position < 0 || h.position >= m_nLen ) 
			return false;
		CXObject* pObj = m_pObjectPool[h.position];
		return ( pObj && ( *(xObject*)&h == pObj->GetObjectID() ) );
	}

	CXObject* CXObjectPool::GetObject( xObject id, xType type )
	{
		object_handle &h = (object_handle&)id;
		if( IsValidObject( id ) && m_pObjectPool[h.position]->IsTypeOf( type ) )
		{
	...
	xObject CXObjectPool::AddObject( CXObject* pObj )
	{
		if( m_nCount >= m_nLen )	return INVALID_OBJECT_ID;
	...
	void CXObjectPool::DelObject( xObject id )
	{
		object_handle& h = (object_handle&)id;
		ASSERT_MSG( m_pObjectPool[h.position] && m_pObjectPool[h.position]->GetObjectID() == id, _T("不等的ID，删除对象时出错。") );
		if( m_pObjectPool[h.position]->GetObjectID() != id )	return;
